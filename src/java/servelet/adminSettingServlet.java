 ...
 * @author Yasiru
 */
@WebServlet(name = "adminSettingServlet", urlPatterns = {"/adminSettingServlet"})
public class adminSettingServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("admin_setting_Password Servlet :username "+user.getUsername()+"");
        try {
            if(UserAccountController.isUserExist(user) ){
                ...
                if((user.getNewPassword_enter1().equals(user.getNewPassword_enter2()))){
                    ...
                    response.sendRedirect("admin_setting_Password.jsp?message=" +URLEncoder.encode("password error", "UTF-8"));
                }
                
            }else{
                //Incorrect username or password
               response.sendRedirect("admin_setting_Password.jsp?message=" +URLEncoder.encode("user account not exist: "+user.getUsername(), "UTF-8"));
            ...
            Logger.getLogger(UserSetting_Serverlet.class.getName()).log(Level.SEVERE, null, ex);
             response.sendRedirect("admin_setting_Password.jsp?message=" + URLEncoder.encode(ex.getMessage(), "UTF-8"));
        }
